...
x <- as.factor(rep(c("WT", "Mutant"), each=4))
names(x) <- colnames(tximport.counts)
filter <- apply(tximport.counts, 1, function(x) length(x[which(x>10)])>5)
filtered <- as.matrix(tximport.counts)[filter,]
